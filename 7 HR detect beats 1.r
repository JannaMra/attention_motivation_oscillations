...
if (exists("path.phys")==F)
  path.phys = paste0(path, "Data/Physio/EXP/") #TODO enter input path here
...
if (exists("trials.n")==F) trials.n = 160 #number of trials that shall be analyzed (if more trials, last ones will be taken); Turn off by assigning: .Machine$integer.max

#output
if (exists("path.rpeaks")==F)
  path.rpeaks = paste0(path,"Data/Physio/rpeaks/")
...
segment.length = 20 # r-peak detection segments in seconds
...
subjects.ecg = list.files(path.phys, pattern=ifelse(unzipFiles, ".zip", "EKG.txt"), full.names=TRUE)
...
#for (index in seq(subjects.ecg)) {
  ...
  data = data %>% mutate(Trigger = ifelse(Trigger <= 3, Trigger, 0))
