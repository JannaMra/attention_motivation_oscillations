...
# add EEG data
eeg <- read.csv2("./Data/EEG_excuding_trials_with_wrong_responses/Auswertung/single_trial_alpha_all_conditions.csv")%>%
  rename(subject = ID)%>%
  select(subject, trial, alpha_bl)%>%
  rename(alpha = alpha_bl)

# add high alpha data
eeg_high <- read.csv2("./Data/EEG_excuding_trials_with_wrong_responses/Auswertung/single_trial_high_alpha_all_conditions.csv")%>%
  rename(subject = ID)%>%
  select(subject, trial, alpha_bl)%>%
  rename(alpha_high = alpha_bl)

# add low alpha data
eeg_low <- read.csv2("./Data/EEG_excuding_trials_with_wrong_responses/Auswertung/single_trial_low_alpha_all_conditions.csv")%>%
  rename(subject = ID)%>%
  select(subject, trial, alpha_bl)%>%
...
model_full_simple <- lmer(rt ~ cb + hr + scl + dilation + alpha_high + cue + trial + (1|subject), All_Data) %>% summary() %>% print() #all
model_full_simple <- lmer(rt ~ cb + hr + scl + dilation + alpha_low + cue + trial + (1|subject), All_Data) %>% summary() %>% print() #all


#performance::r2(model_full_simple)
model_full <- lmer(rt ~ cb + hr + scl + dilation + alpha + cue + trial + cb:cue + hr:cue + scl:cue + dilation:cue + alpha:cue + (1|subject), All_Data) %>% summary() %>% print() #all
...
lmer(rt ~ cb + hr + scl + dilation + alpha_high  + (1|subject), All_Data %>% filter(cue == 2))%>% summary() %>% print() #flight painful
lmer(rt ~ cb + hr + scl + dilation + alpha_high  + (1|subject), All_Data %>% filter(cue == 4))%>% summary() %>% print() #flight non-painful

lmer(rt ~ cb + hr + scl + dilation + alpha_low  + (1|subject), All_Data %>% filter(cue == 2))%>% summary() %>% print() #flight painful
lmer(rt ~ cb + hr + scl + dilation + alpha_low  + (1|subject), All_Data %>% filter(cue == 4))%>% summary() %>% print() #flight non-painful
